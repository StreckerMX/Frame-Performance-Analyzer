...
public class ChartViewportFullBoundsTests
{
    ...
    public void Full_limits_span_the_entire_loaded_session_range()
    {
        ...
        var limits = ChartViewport.FullSeriesLimits([session], fpsBaselineZero: true);

        Assert.NotNull(limits);
        Assert.Equal(0, limits.Value.Left, precision: 9);
        Assert.Equal(150, limits.Value.Right, precision: 9);
        Assert.Equal(0, limits.Value.Bottom, precision: 9);
    ...
    public void Full_limits_do_not_depend_on_any_viewport()
    {
        ...
        // full session.
        var full = ChartViewport.FullSeriesLimits([session], fpsBaselineZero: true);
        var afterNarrowView = ChartViewport.FullSeriesLimits([session], fpsBaselineZero: true);
    ...
    {
        Assert.Null(ChartViewport.FullSeriesLimits([], fpsBaselineZero: false));
    }
    ...
    public void Non_fps_metrics_pad_below_zero_baseline()
    {
        ...
        var limits = ChartViewport.FullSeriesLimits([series], fpsBaselineZero: false);
