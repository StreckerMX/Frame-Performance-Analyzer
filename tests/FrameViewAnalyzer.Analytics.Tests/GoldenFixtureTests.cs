...
/// <summary>
/// Certified parity against the Python reference application. The fixture
/// set (Fixtures/golden_*.csv + golden.json) is generated by
/// tools/generate_golden_fixture.py, which runs the Python app's own
/// analytics over the same CSVs. Regenerate the fixture only after a
/// deliberate behavior change, and treat any mismatch here as a bug.
/// </summary>
public class GoldenFixtureTests
{
    ...
    private static void AssertSession(SessionAnalysis session, JsonElement expected)
    {
        ...
        for (var i = 0; i < expectedMetrics.Count; i++)
        {
            ...
            {
                AssertClose(expectedPoints[p][0].GetDouble(), series.X[p]);
