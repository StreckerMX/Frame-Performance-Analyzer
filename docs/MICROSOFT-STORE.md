...
Frame Performance Analyzer is published to GitHub as a self-contained single-file Windows app. For Microsoft Store distribution, the same application payload is wrapped in an MSIX package so the Store can certify and sign it.
...
- `packaging/store/StoreIdentity.json` stores the public Partner Center identity assigned to Frame Performance Analyzer.
...
The script resolves the product version from `Directory.Build.props`. Version `3.2.0` becomes MSIX package version `3.2.0.0`; the fourth component stays `0` for Microsoft Store use.

Output:

```text
artifacts/store/FramePerformanceAnalyzer-Store-3.2.0.0-x64.msix
artifacts/store/FramePerformanceAnalyzer-Store-3.2.0.0-x64.msix.sha256
```
...
The manifest intentionally retains `FrameViewAnalyzer` as its internal Application Id and `FrameViewAnalyzer.exe` as its packaged executable name. Those values belong to the previously validated technical package structure; the Store-facing display name and physical MSIX filename use **Frame Performance Analyzer**.

## Partner Center submission checklist

After the production MSIX is generated:

1. On the reserved **Frame Performance Analyzer** product overview, select **Start submission**.
...
5. Upload `FramePerformanceAnalyzer-Store-3.2.0.0-x64.msix` on the **Packages** page.
...
- App `3.2.1` → MSIX `3.2.1.0`
- App `3.3.0` → MSIX `3.3.0.0`
