...
.DESCRIPTION
    Publishes Frame Performance Analyzer as the existing self-contained single-file app,
...
if ($PackageVersion -notmatch '^\d+\.\d+\.\d+\.0$') {
    throw "PackageVersion must use four numeric components and end in .0 (example: 3.2.0.0). Got '$PackageVersion'."
}

Write-Host "==> Frame Performance Analyzer Store package $PackageVersion"
...
$outputName = "FramePerformanceAnalyzer-Store-$PackageVersion-x64.msix"
